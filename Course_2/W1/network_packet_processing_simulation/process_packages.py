        ...
        if len(self.finish_time) == 0:
            ...
            if len(self.finish_time) >= self.size:
                ...
                if len(self.finish_time) == 0:
                    ...
                    return Response(False, time_start)
...
def main():
    ...
    for i in range(19, 23):
        with open('./tests/' + '{:02d}'.format(i), 'r') as inputt:
            buffer_size, n_requests = map(int, inputt.readline().split())
            requests = []
            for _ in range(n_requests):
                arrived_at, time_to_process = map(int, inputt.readline().split())
                requests.append(Request(arrived_at, time_to_process))
            buffer = Buffer(buffer_size)
            responses = process_requests(requests, buffer)
            
        with open('./' + '{:02d}'.format(i), 'w+') as output:
            for response in responses:
                output.write(str(response.started_at) + '\n')
                #print(response.started_at if not response.was_dropped else -1)
